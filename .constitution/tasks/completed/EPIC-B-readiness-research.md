...
- **OXY-B003** — `.constitution/spikes/SPK-B003.md`: chose a mix of A, B, and C — freeze the source-level Wayland core, shell, scale, text-input, clipboard, and presentation floors; use the shared Ubuntu `libgtk-4-1` `4.22.2+ds-1ubuntu1` and `at-spi2-core` `2.60.0-1` package identities for both Linux reference rows while retaining the `gtk4` crate `v4_20` API-binding ceiling and AT-SPI 2.60.6 source floor as distinct constraints; adopt the DRM `drm:drm_vblank_event` design as prospective only; retain every reference-session and candidate-specific row. Table 1 records 9 KK rows and 12 KU (gating) rows.
