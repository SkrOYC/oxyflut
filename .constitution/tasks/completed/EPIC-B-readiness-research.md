...
- **OXY-B008** — `.constitution/reports/qualification-assessors.md`: BLOCKED. The ticket pass rule requires two distinct preserved confirmations and only assessor 1 is recorded, so the acceptance evidence is explicitly deferred rather than claimed. Assessor 2 is unnamed, and assessor 1's disclosed candidate-code and qualification-evidence authorship is a gating conflict until Stage 1 approves an authorship-independence policy.

### Deviations & Justifications

- **OXY-B008 acceptance not met.** The ticket's `hitl_sil` pass log requires two distinct confirmations preserved before candidate implementation. `.constitution/reports/qualification-assessors.md` preserves one confirmation (Oscar Y. <oscar@ocmasesorias.com>) with full conflict disclosures and freezes the second-assessor confirmation procedure, the independence rules, the evidence-access procedure, and the written-consensus procedure. Its Q4 row is the ticket's own STOP condition, so the missing confirmation is recorded as a blocked external input rather than a technical completion. OXY-B008 stays open in spirit: its remaining work is routed to `OXY-D001` as a named blocked external input, and no ticket in this epic may set `measurementPolicy.assessors`, `measurementPolicy.scoringAnchors`, or `candidateImplementationReady`.
- **OXY-B007 macOS and Windows rows blocked.** The ticket's STOP condition requires an environment with no accountable owner or access procedure to be recorded as blocked. `.constitution/reports/reference-hardware-access.md` records B007-Q01, B007-Q02, B007-Q05, B007-Q06, B007-Q09, and B007-Q10 as gating KUs and marks both rows BLOCKED in the reference-conformance table. Its `Spec edits required` section authorizes no Stage 3 edit.
- **Linux rows come from a non-reference host and count as one configuration.** The confirmed Wayland and X11 rows are the single physical machine `thinkpadp14s`, an x86_64 NixOS 26.05 host with a Hyprland Wayland session and an Xwayland or Xvfb X11 path, not the Stage 3 Ubuntu 26.04 LTS reference. The ticket's STOP condition forbids describing one machine as two independent hardware configurations, so B007-Q11 and B007-Q12 record that this register supplies no second-configuration score-4 evidence, and B007-Q07 and B007-Q08 record the reference-conformance gap.
- **OXY-B006 selected the synthetic patch after the real-patch search failed.** P1 in `.constitution/spikes/SPK-B006.md` resolved the engine pins for all three frozen Flutter lines and fetched their actual `pngrtran.c` files; both pins already contain the `08da33b` libpng postimage, so the candidate real patch has no remaining preimage and cannot rehearse remediation. The unpinned integrated fork also prevents proving a shared real-patch path. That triggers the ticket's real-upstream-patch STOP condition only, so the spike selected option B, the predeclared synthetic patch `OXY-SYN-SEC-001`.
