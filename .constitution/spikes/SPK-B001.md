...
```sh
repo_root="${OXYFLUT_REPO_ROOT:?set to the checkout path}"
workdir=/tmp/wf-epic-b/OXY-B001/mac-ime-probe
...
| File and field or section | Exact instruction |
| :-- | :-- |
| `.constitution/tech-spec/data-models/semantic-role-registry.schema.json` -> new file | Create a Draft 2020-12 schema with `$id` `urn:oxyflut:schema:semantic-role-registry:1`, `additionalProperties: false`, and a `properties` entry `"$schema": {"type":"string"}`. Require `schemaVersion`, `codeStabilityRule`, `roleVocabularySources`, and `roles`. Set `schemaVersion` to `1.0.0` and `codeStabilityRule` to `u32-append-only-never-reuse-v1`. Require exactly three `roleVocabularySources` records with `environment`, `vocabulary`, and `url` strings. Require `roles` to be a nonempty array sorted by `name`; require validator-enforced unique `name` and `code`. Each role record has `additionalProperties: false` and required `name`, `code`, `ax`, `uia`, and `atspi`: `name` matches `^[a-z][a-z0-9-]*$`; `code` is an integer from `0` through `4294967295`; `ax` is an object with only required nonempty `role` string and nullable `subrole` string; `uia` and `atspi` are nonempty strings. |
| `qualification/fixtures/contracts/semantic-role-registry/` -> new schema-fixture directory | Create `valid/minimal.json`; create `invalid/additional-properties.json`, `invalid/conditional.json`, `invalid/enum.json`, `invalid/required.json`, and `invalid/type.json`; and create the matching `invalid/additional-properties.expected.json`, `invalid/conditional.expected.json`, `invalid/enum.expected.json`, `invalid/required.expected.json`, and `invalid/type.expected.json` sidecars. The valid fixture must validate against `semantic-role-registry.schema.json`. Each invalid fixture must fail only its declared JSON Schema rule, and its matching sidecar must declare that validation failure's stable instance path and keyword. Create this directory in the same change as the schema: `run_fixture_corpus` requires the schema-fixture directory set to equal the data-model schema-name set, so its absence is a `FixtureSchema` failure. |
...
| `qualification/fixtures/contracts/semantic-role-registry-snapshot/` -> new schema-fixture directory | Create `valid/minimal.json`; create `invalid/additional-properties.json`, `invalid/conditional.json`, `invalid/enum.json`, `invalid/required.json`, and `invalid/type.json`; and create the matching `invalid/additional-properties.expected.json`, `invalid/conditional.expected.json`, `invalid/enum.expected.json`, `invalid/required.expected.json`, and `invalid/type.expected.json` sidecars. The valid fixture must validate against `semantic-role-registry-snapshot.schema.json`. Each invalid fixture must fail only its declared JSON Schema rule, and its matching sidecar must declare that validation failure's stable instance path and keyword. Create this directory in the same change as the schema: `run_fixture_corpus` requires the schema-fixture directory set to equal the data-model schema-name set, so its absence is a `FixtureSchema` failure. |
| `.constitution/tech-spec/contracts/semantic-role-registry.json` -> new file | Declare `"$schema": "urn:oxyflut:schema:semantic-role-registry:1"`, then create a registry with `schemaVersion` equal to `1.0.0` and `codeStabilityRule` equal to `u32-append-only-never-reuse-v1`. The URN self-declaration is required because `discover_contract_instances` rejects a contract instance without `$schema` and resolves `urn:` references through the local registry. Its `urn:oxyflut:schema:<schema-name>:<major>` form matches the existing `platform-contracts.schema.json` `$id`; `contracts/platform-contracts.json` demonstrates the existing contract-instance `$schema` member. Set exactly three `roleVocabularySources`: macOS `NSAccessibility.Role` at https://developer.apple.com/documentation/appkit/nsaccessibility-swift.struct/role, Windows `UI Automation Control Types` at https://learn.microsoft.com/en-us/windows/win32/winauto/uiauto-controltypesoverview, and Wayland and X11 `AtspiRole` at https://gnome.pages.gitlab.gnome.org/at-spi2-core/devel-docs/doc-org.a11y.atspi.Accessible.html. Set `roles` to a nonempty array sorted by `name`. Each exact record must contain the Stage 3 crosswalk's lower-kebab-case `name`, its stable unique `u32` `code`, `ax` with the selected `NSAccessibility.Role` and nullable selected `NSAccessibility.Subrole`, `uia` with the selected UI Automation Control Type, and `atspi` with the selected `AtspiRole`. Don't derive, add, remove, renumber, or reuse a role or code from either candidate implementation or a platform map. |
...
| `qualification/fixtures/contracts/semantic-role-registry/` | Create `valid/minimal.json`; create `invalid/additional-properties.json`, `invalid/conditional.json`, `invalid/enum.json`, `invalid/required.json`, and `invalid/type.json`; and create matching `.expected.json` sidecars for all five invalid files. The valid fixture must validate against `semantic-role-registry.schema.json`; each invalid fixture must fail only its declared JSON Schema rule and have a sidecar with that failure's stable instance path and keyword. Land this directory atomically with `semantic-role-registry.schema.json`, because `xtask/src/contracts/schema.rs` `run_fixture_corpus` rejects a schema-name and fixture-directory mismatch as `FixtureSchema`. |
| `qualification/fixtures/contracts/semantic-role-registry-snapshot/` | Create `valid/minimal.json`; create `invalid/additional-properties.json`, `invalid/conditional.json`, `invalid/enum.json`, `invalid/required.json`, and `invalid/type.json`; and create matching `.expected.json` sidecars for all five invalid files. The valid fixture must validate against `semantic-role-registry-snapshot.schema.json`; each invalid fixture must fail only its declared JSON Schema rule and have a sidecar with that failure's stable instance path and keyword. Land this directory atomically with `semantic-role-registry-snapshot.schema.json`, because `xtask/src/contracts/schema.rs` `run_fixture_corpus` rejects a schema-name and fixture-directory mismatch as `FixtureSchema`. |
...
| `.constitution/tech-spec/data-models/README.md` | Add one table row for `semantic-role-registry.schema.json` with purpose `Defines the candidate-neutral closed semantic-role registry and platform vocabulary crosswalk.` and compatibility rule `Role codes are append-only and never reused; a change, deletion, or reuse requires a compatibility migration.` Add one table row for `semantic-role-registry-snapshot.schema.json` with purpose `Records the immutable, validated snapshot of the candidate-neutral semantic-role registry.` and compatibility rule `The snapshot preserves its source-registry path and SHA-256 and must have an exact role and vocabulary-source match.` Replace `accessibility-map v5` in the supersession summary with `accessibility-map v6` and state that v6 supersedes v5 because `forward.roles` is now a keyed role map with registry provenance. Retain the pre-evidence preservation statement. |
