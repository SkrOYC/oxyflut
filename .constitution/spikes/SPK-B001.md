...
The exact CJK input-source identity is KU (gating). S23 says that Text Input Source Services identifies sources by `kTISPropertyInputSourceID` and uses `com.apple.inputmethod.Kotoeri.Japanese` only as an example. It doesn't establish an identifier present on the qualification host. Before V5 runs, P1a must enumerate `TISCreateInputSourceList` on the pinned arm64 macOS host, recording every enabled source ID, localized name, `kTISPropertyInputSourceType`, and `kTISPropertyInputSourceLanguages` list, plus `sw_vers`, `xcodebuild -version`, and `uname -m`, in `input-sources.json`. P1a must write the SHA-256 of that exact JSON beside the output. The selection matcher must select only an enabled source whose `kTISPropertyInputSourceType` equals `kTISTypeKeyboardInputMode` or `kTISTypeKeyboardInputMethodModeEnabled`; it must reject `kTISTypeKeyboardLayout`. The matcher must parse every `kTISPropertyInputSourceLanguages` entry as a BCP-47 language tag and compare its case-insensitive primary language subtag to `zh`, `ja`, or `ko`, rather than comparing a display name or a locale string. P1 must call `TISSelectInputSource` for the selected source, then call `TISCopyCurrentKeyboardInputSource` and require its `kTISPropertyInputSourceID` to equal the selected ID before V5 begins. P1 must record the selected and active IDs, type, matching language tag, and inventory digest in every V5 transcript. It must preserve a preliminary `setMarkedText` callback for that active source before running any V5 matrix cell. If selection, active-ID confirmation, or that callback fails, P1a emits `input-source-missing`, `input-source-activation-mismatch`, or `cjk-composition-not-observed`; P1 does not run V5, and B001-04 remains KU. P1 must not substitute an unenumerated ID. The P1 implementation must preserve the Apple Text Input Source Services declaration excerpts for every TIS property, type constant, and function named in this procedure beside its transcript; S23 is the fetched Apple source for source IDs. See S23.

P1a command: `cd /tmp/wf-epic-b/OXY-B001/mac-ime-probe && xcrun --sdk macosx clang -fobjc-arc -framework AppKit -framework Carbon ime_input_source_inventory.m -o ime_input_source_inventory && ./ime_input_source_inventory --enabled --selected-cjk --require-input-source-types kTISTypeKeyboardInputMode,kTISTypeKeyboardInputMethodModeEnabled --bcp47-primary-language zh,ja,ko --output input-sources.json && shasum -a 256 input-sources.json`.
...
After P1a records the selected ID, the P1 command is `cd /tmp/wf-epic-b/OXY-B001/mac-ime-probe && xcrun --sdk macosx clang -fobjc-arc -framework AppKit -framework Carbon ime_probe.m -o ime_probe && ./ime_probe --two-views --input-source "$(jq -r '.selectedCjkInputSource.id' input-sources.json)" --input-source-inventory input-sources.json --require-input-source-types kTISTypeKeyboardInputMode,kTISTypeKeyboardInputMethodModeEnabled --confirm-current-keyboard-input-source --require-preliminary-set-marked-text --matrix ime-matrix.json --jsonl transcript.jsonl`. The command must exit nonzero if the selected ID is absent or disabled, has `kTISTypeKeyboardLayout` or another disallowed type, lacks a valid matching `zh`, `ja`, or `ko` BCP-47 primary language subtag, differs from the active ID returned by `TISCopyCurrentKeyboardInputSource`, lacks the preliminary `setMarkedText` callback, or differs from the inventory digest recorded in a V5 line.
...
The inspected candidate-neutral inputs don't enumerate a role identifier. `.constitution/prd/capabilities.md` gives CAP-SEM-001's role-specific coverage and CAP-SEM-002's reverse-action requirement, while the two semantics flows define mapping, update, action, and stale-target behavior. `accessibility-map.schema.json` requires the `forward.roles` mapping but leaves its `oxyflut` and `native` strings unrestricted. Therefore an `explicit-role-values-v1` freeze over those inputs necessarily yields an empty set. It must emit `role-registry-empty`; it must not treat that empty result as complete coverage.

D0 is an upstream Stage 3 decision, not a P2R probe. Before P2R, Stage 3 must add the candidate-neutral `contracts/semantic-role-registry.json` contract and its schema specified in [Spec edits required](#downstream-impact). Stage 3 must choose the closed Oxyflut role set from a recorded crosswalk of Apple `NSAccessibility.Role`, Microsoft UI Automation Control Types, and AT-SPI `AtspiRole`; native vocabularies inform the crosswalk but don't let either candidate define or expand the set. Apple documents named `NSAccessibility.Role` symbols, Microsoft describes UI Automation control types as well-known control identifiers, and AT-SPI identifies `AtspiRole` as its role-value enumeration. See S28, S29, and S30.

A `roles` enumeration added directly to `accessibility-map.schema.json` is rejected. That schema defines the structure of a per-environment candidate map, and its existing `forward.roles` value is a mapping object rather than the semantic catalog. An enumeration there would conflate the candidate-neutral semantic contract with platform map validation, provide no source-vocabulary provenance, and require every map-schema revision to change the closed role set. The selected `contracts/semantic-role-registry.json` plus `data-models/semantic-role-registry.schema.json` pair isolates the closed catalog, its vocabulary sources, and its contract version. P2 still validates each candidate map against the unchanged complete `accessibility-map.schema.json` and separately validates coverage of every frozen registry role.

After D0 succeeds, P2R must validate the source contract, then write the immutable snapshot at `qualification/fixtures/external-contracts/accessibility/candidate-neutral-role-registry.json` and its sidecar digest at `qualification/fixtures/external-contracts/accessibility/candidate-neutral-role-registry.json.sha256`. The snapshot must contain `schemaVersion`, `sourceRegistryPath`, `sourceRegistrySha256`, `roleEnumerationRule`, `roleVocabularySources`, and the sorted nonempty `roles` array. The rule is `stage3-semantic-role-registry-v1`: copy only the declared `roles` array from the valid Stage 3 registry; sort bytewise; reject duplicates; don't infer a role from a field name, a native platform role, or either candidate. P2R must validate it with `registry=/home/oscar/GitHub/oxyflut/qualification/fixtures/external-contracts/accessibility/candidate-neutral-role-registry.json && jq -e '.schemaVersion == "1.0.0" and .sourceRegistryPath == ".constitution/tech-spec/contracts/semantic-role-registry.json" and (.sourceRegistrySha256 | test("^[0-9a-f]{64}$")) and .roleEnumerationRule == "stage3-semantic-role-registry-v1" and (.roleVocabularySources | type == "array" and length == 3) and (.roles | type == "array" and length > 0 and . == (sort | unique) and all(.[]; test("^[a-z][a-z0-9-]*$")))' "$registry" && shasum -a 256 -c "$registry.sha256"`. It must fail with `role-registry-invalid` if the Stage 3 registry or schema is absent or invalid, the copied source digest differs, a role is missing or duplicated, a vocabulary source differs, or a candidate or native role influences the set. P2 must not start before this validation succeeds.
...
| :-- | :-- | :-- |
| P1 | P1a and `mac-ime-probe` commands in [Input method editor qualification corpus](#input-method-editor-qualification-corpus). | P1a enumerates and digests enabled host input sources, admits only the stated composition-capable types and BCP-47 language match, calls `TISSelectInputSource`, confirms the active ID with `TISCopyCurrentKeyboardInputSource`, and captures the required SDK declarations. P1 runs V5 only after the selected source emits preliminary `setMarkedText`; otherwise it records the named activation failure and retains the KU. |
| D0 | Stage 3 semantic-role decision specified in [Spec edits required](#downstream-impact). | Adds and validates the candidate-neutral closed role registry from the recorded AX, UIA, and AT-SPI vocabulary crosswalk. This decision must complete before P2R; it is not a candidate probe. |
| P2R | `cd /tmp/wf-epic-b/OXY-B001/mac-accessibility-registry && ./validate-and-freeze-role-registry.sh --registry /home/oscar/GitHub/oxyflut/.constitution/tech-spec/contracts/semantic-role-registry.json --schema /home/oscar/GitHub/oxyflut/.constitution/tech-spec/data-models/semantic-role-registry.schema.json --output /home/oscar/GitHub/oxyflut/qualification/fixtures/external-contracts/accessibility/candidate-neutral-role-registry.json`. | Validates D0's contract and source vocabulary records, writes the snapshot and SHA-256 only when its nonempty role array passes, and otherwise emits `role-registry-invalid`. |
| P2 | `mac-accessibility-probe` command in [Accessibility qualification corpus](#accessibility-qualification-corpus), after D0 and P2R. | Verifies the D0 registry snapshot digest, validates focused and integrated outputs against the same complete schema, then records VoiceOver traversal and every declared reverse action. |
...
- **Spec edits required:** B001-02 remains a gating KU, so Stage 3 must retain the deployment-target state until P8 produces one verified maximum-minimum record. The four atomic deployment-target edits are listed first. The accessibility-KU guard and D0 semantic-role prerequisite are separate requirements.
...
| File and field or section | Exact instruction |
| :-- | :-- |
| `.constitution/tech-spec/data-models/semantic-role-registry.schema.json` -> new file | Create a Draft 2020-12 schema with `$id` `urn:oxyflut:schema:semantic-role-registry:1`, `additionalProperties: false`, and required `schemaVersion`, `roleVocabularySources`, and `roles` fields. Set `schemaVersion` to the constant `1.0.0`. Require exactly three `roleVocabularySources` records with `environment`, `vocabulary`, and `url` strings. Require `roles` to be a nonempty unique array of lower-kebab-case strings matching `^[a-z][a-z0-9-]*$`. |
| `.constitution/tech-spec/contracts/semantic-role-registry.json` -> new file | Create a registry with `schemaVersion: "1.0.0"` and exactly three `roleVocabularySources`: `{ "environment": "macos", "vocabulary": "NSAccessibility.Role", "url": "https://developer.apple.com/documentation/appkit/nsaccessibility-swift.struct/role" }`, `{ "environment": "windows", "vocabulary": "UI Automation Control Types", "url": "https://learn.microsoft.com/en-us/windows/win32/winauto/uiauto-controltypesoverview" }`, and `{ "environment": "wayland-and-x11", "vocabulary": "AtspiRole", "url": "https://gnome.pages.gitlab.gnome.org/at-spi2-core/devel-docs/doc-org.a11y.atspi.Accessible.html" }`. Set `roles` to the nonempty, sorted, unique lower-kebab-case closed Oxyflut role list selected by the explicit Stage 3 crosswalk of those three vocabularies. Don't derive, add, or remove a role from either candidate implementation or a platform map. |
| `.constitution/tech-spec/contracts/capability-traceability.json` -> mappings for `CAP-SEM-001` and `CAP-SEM-002` -> `bindings` | Add `{ "contract": "contracts/semantic-role-registry.json", "symbols": ["#/roles", "#/roleVocabularySources"] }` to each capability's bindings. |
