...
The focused P1 command is `repo_root=/home/oscar/GitHub/oxyflut && workdir=/tmp/wf-epic-b/OXY-B001/mac-ime-probe && selection="$workdir/output/input-source-selection.json" && cd "$workdir" && mkdir -p output/focused && xcrun --sdk macosx clang -fobjc-arc -framework AppKit -framework Carbon ime_focused_probe.m -o ime_focused_probe && ./ime_focused_probe --allocation focused --two-views --input-source-selection "$selection" --input-source-inventory output/input-sources.json --require-preliminary-set-marked-text --matrix ime-matrix.json --jsonl output/focused/transcript.jsonl --validation output/focused/validation.json`. It exits `0` only when the preliminary callback and every matrix validation pass, `30` with `cjk-composition-not-observed` in `output/focused/validation.json` before V5, or `31` with `focused-ime-validation-failed` in that file after a matrix failure.

The integrated P1 command runs only after P6: `repo_root=/home/oscar/GitHub/oxyflut && workdir=/tmp/wf-epic-b/OXY-B001/mac-ime-probe && lock=/tmp/wf-epic-b/OXY-B001/integrated-inventory/integrated-input-lock.json && selection="$workdir/output/input-source-selection.json" && cd "$workdir" && mkdir -p output/integrated && ./run-integrated-ime-probe --input-lock "$lock" --allocation integrated --two-views --input-source-selection "$selection" --input-source-inventory output/input-sources.json --c-abi-contract "$repo_root/.constitution/tech-spec/contracts/oxyflut-substrate.h" --require-nstextinputclient-through-oxy-platform-event --require-preliminary-set-marked-text --matrix ime-matrix.json --jsonl output/integrated/transcript.jsonl --validation output/integrated/validation.json`. The runner must validate the P6 lock before it initializes either view, including presence, both frozen commits, and both 64-hex source digests. It exits `0` only when the P6 input lock, C-ABI route, preliminary callback, and every matrix validation pass; it exits `40` with `integrated-input-lock-invalid`, `41` with `nstextinputclient-c-abi-route-missing`, `42` with `cjk-composition-not-observed` before V5, or `43` with `integrated-ime-validation-failed`, always writing `output/integrated/validation.json`. P1 must retain B001-04 as KU on every nonzero result.
...
| :-- | :-- | :-- |
| P1 | P1a and separate focused and integrated commands in [Input method editor qualification corpus](#input-method-editor-qualification-corpus). | P1a inventories, selects, and confirms the active source, then writes `output/input-sources.json` and `output/input-source-selection.json` with exit `0`, `20`, `21`, or `22`. P1 records preliminary callback and matrix results only in allocation-specific `output/focused/` and `output/integrated/` transcripts and validation files. The focused run is standalone. The integrated run follows P6, binds its fork and adapter digests, and records only after `OxySubstrateCallbacks.on_platform_event` receives the translated input method editor event. |
...
| P6 | `repo_root=/home/oscar/GitHub/oxyflut && cd /tmp/wf-epic-b/OXY-B001/integrated-inventory && ./inventory.sh --qualification-lock "$repo_root/.constitution/tech-spec/contracts/qualification-lock.json" --inventory-output inventory.json --input-lock-output integrated-input-lock.json`. | Reads the frozen integrated-fork and Oxyflut-adapter commits from the qualification lock, computes and records their source SHA-256 values in `integrated-input-lock.json`, and emits a commit-bound macOS path and symbol inventory for input method editor, accessibility, clipboard, timing, lifecycle, Metal error, and C-ABI crossings. It exits `0` only with both frozen commits and digests; it exits `80` with `integrated-input-pin-missing` or `81` with `integrated-inventory-incomplete`. |
...
| `.constitution/tech-spec/data-models/accessibility-map.schema.json` -> schema identity, root `schemaVersion`, and `forward.roles` | Perform one breaking compatibility migration: change `$id` from `urn:oxyflut:schema:accessibility-map:5` to `urn:oxyflut:schema:accessibility-map:6` and the root `schemaVersion` constant from `5.0.0` to `6.0.0`. Add `forward.properties.roles` as an object with `minProperties: 1`, `propertyNames.pattern` equal to `^[a-z][a-z0-9-]*$`, and `additionalProperties` referencing `#/$defs/mapping`. Replace the broad `forward.patternProperties` with a pattern that matches every required single-mapping field except `roles`, so `roles` isn't also validated as one `mapping`. Keep `mapping` unchanged. Add version-5-to-version-6 migration fixtures and contract tests. |
