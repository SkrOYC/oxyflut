...
Hardware access is a precondition: the [reference hardware access register](../reports/reference-hardware-access.md#answers) records macOS arm64 and Windows x86-64 as blocked under `B007-Q01` and `B007-Q02`, with neither an accountable owner nor a usable access procedure. This 48-tuple probe isn't actionable until both rows are unblocked.
...
- `xtask/src/contracts/schema.rs` in `schema_compiles_committed_contract_instances_and_fixture_corpus`: change `assert_eq!(report.schema_count, 18);` to `assert_eq!(report.schema_count, 23);` and `assert_eq!(report.instance_count, 6);` to `assert_eq!(report.instance_count, 7);`. OXY-D001 composes this spike's `+3` schemas and `+0` instances (the schemas back fixtures only) with SPK-B001's `+2` schemas and `+1` instance.
- `.constitution/tech-spec/data-models/README.md` in the `Durable qualification data` table: after `ingress-inventory.schema.json`, add the following three rows in the existing `Schema | Purpose | Compatibility rule` format:

| Schema | Purpose | Compatibility rule |
| :-- | :-- | :-- |
| `layout-prequalification-run.schema.json` | Records one complete, unscored CON-PERF-001 layout-cap probe for one candidate, environment, and passing corpus fixture. | Preserve earlier run documents; breaking changes require a major schema version. |
| `layout-prequalification-suite.schema.json` | Records the complete unscored layout-cap matrix. | Preserve earlier suite documents; breaking changes require a major schema version. |
| `layout-qualification-record.schema.json` | Records one raw, harness-emitted root layout transaction. | Preserve earlier record documents; breaking changes require a major schema version. |
...
| `qualification/fixtures/contracts/layout-qualification-record/` | N: add the schema-valid and schema-invalid matrix for the layout-qualification-record contract. |
| `qualification/fixtures/contracts/layout-prequalification-run/` | N: add the schema-valid and schema-invalid matrix for the layout-prequalification-run contract. |
