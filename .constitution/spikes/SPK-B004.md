...
- Round-10 correction clock start / stop: 2026-08-29T01:32:29Z / 2026-08-29T01:42:21Z.
- Round-12 correction clock start / stop: 2026-08-29T02:32:25Z / 2026-08-29T02:37:51Z.
...
The [GTK 4.22.2 text bridge source](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/a11y/gtkatspitext.c) selects the generic `GtkAccessibleText` vtable before the editable vtable. It contains the following selector:
...
For the primary `GtkAccessibleText` path, the [GTK 4.22.2 text bridge source](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/a11y/gtkatspitext.c) requests the one-character range and decodes the returned UTF-8 sequence:
...
The GTK 4.22.2 `GtkAccessibleText` API documentation defines `get_contents` with "@start: the beginning of the range, in characters" and "@end: the end of the range, in characters," and returns the requested slice as UTF-8. Its implementation calculates the character count from the complete `get_contents (self, 0, G_MAXUINT)` result with `len = g_utf8_strlen (str, -1);`. The bridge exports that helper as `CharacterCount`. [GTK 4.22.2 GtkAccessibleText API documentation](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/gtkaccessibletext.h) and the [GTK 4.22.2 accessible text implementation](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/gtkaccessibletext.c) establish the primary path's character-range and character-count units.

The `GtkEditable` branch is a fallback, not the universal provider implementation. The [GTK 4.22.2 text bridge source](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/a11y/gtkatspitext.c) gives its `GetCharacterAtOffset` branch the separate explicit conversion:
...
The following mutable `nixpkgs#python3` transcript is historical evidence. It remains unchanged and isn't the current procedure.
...
The Round-12 current procedure reran the same report-preserved fixture from `/tmp/wf-epic-b/OXY-B004/round-12/unicode_offsets.py` with the CI and `devenv.lock` nixpkgs revision. The output below is trimmed to the success result.

```text
$ nix shell github:NixOS/nixpkgs/56c02bc00adcf003215cc4bd996d6efaf4cff188#python312 --command python3 /tmp/wf-epic-b/OXY-B004/round-12/unicode_offsets.py
all forward and inverse conversions passed
exit=0
```

### Round-10 commit-pinned retrieval verification

The following direct canonical retrieval check verifies the commit-pinned URLs used by numbered edits 4 through 6, table 3, and the Sources list. The recorded SHA-256 values are full response-body digests from direct raw or cgit downloads in `/tmp/wf-epic-b/OXY-B004/round-10/`; they are probe evidence only and do not replace the preservation-step fixture digests.

```text
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/gtkatspitext.c -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/a11y/gtkatspitext.c
http=200
bytes=37061 sha256=65178d6d816cd7b1d91cdea177949aca249469d1b7b0072221e0fa1bb65b9e66
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/gtkaccessibletext.c -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/gtkaccessibletext.c
http=200
bytes=22443 sha256=4efeb6bf88a05d47eb98fe69bc5905c5208343890a980b1fe89d7aaf99ff6e6e
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/gtkaccessibletext.h -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/gtkaccessibletext.h
http=200
bytes=20380 sha256=a9dbead627c1b5abffed3aaaa2a6b2e49c7350c100e8349dffddc46ece79faab
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/gtk-COPYING -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/COPYING
http=200
bytes=25383 sha256=b7993225104d90ddd8024fd838faf300bea5e83d91203eab98e29512acebd69c
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/atspi-Text.xml -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/xml/Text.xml
http=200
bytes=25182 sha256=602cdb27666912ac0cdf9ac53e5d718e002cd4fe1a37e9a9dc67c71f2acc4249
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/atspi-COPYING -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/COPYING
http=200
bytes=26530 sha256=dc626520dcd53a22f727af3ee42c770e56c97a64fe3adb063799d8ab032fe551
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/presentproto.txt -w 'http=%{http_code}\n' 'https://cgit.freedesktop.org/xorg/proto/presentproto/plain/presentproto.txt?id=bfdc7e052302c79c5803ad95a73c9b63b350c40c'
http=200
bytes=22158 sha256=3235ed07ee20958bcc8aed49b5ab341260002cc0f37f57fe369264421cad62ea
$ curl -sS -L --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-10/present-COPYING -w 'http=%{http_code}\n' 'https://cgit.freedesktop.org/xorg/proto/presentproto/plain/COPYING?id=bfdc7e052302c79c5803ad95a73c9b63b350c40c'
http=200
bytes=1128 sha256=64a579d1a0b3347301d87c94698a58ea7933d0ae6ca7e8fdd702fde2ea376301
```

### Round-12 repository and retrieval alignment

The direct GitLab retrieval below confirms that the `s07` source's declared repository, repository-relative path, and retrieval URL identify the same commit-pinned object. The historical cgit retrieval remains comparison evidence only. Both responses are 22,158 bytes, have SHA-256 `3235ed07ee20958bcc8aed49b5ab341260002cc0f37f57fe369264421cad62ea`, and are byte-identical.

```text
$ curl -sS -fL --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-12/presentproto-gitlab.txt -w 'gitlab-http=%{http_code}\n' https://gitlab.freedesktop.org/xorg/proto/presentproto/-/raw/bfdc7e052302c79c5803ad95a73c9b63b350c40c/presentproto.txt
$ curl -sS -fL --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-12/presentproto-cgit.txt -w 'cgit-http=%{http_code}\n' 'https://cgit.freedesktop.org/xorg/proto/presentproto/plain/presentproto.txt?id=bfdc7e052302c79c5803ad95a73c9b63b350c40c'
gitlab-http=200
cgit-http=200
$ wc -c /tmp/wf-epic-b/OXY-B004/round-12/presentproto-gitlab.txt /tmp/wf-epic-b/OXY-B004/round-12/presentproto-cgit.txt
22158 /tmp/wf-epic-b/OXY-B004/round-12/presentproto-gitlab.txt
22158 /tmp/wf-epic-b/OXY-B004/round-12/presentproto-cgit.txt
44316 total
$ sha256sum /tmp/wf-epic-b/OXY-B004/round-12/presentproto-gitlab.txt /tmp/wf-epic-b/OXY-B004/round-12/presentproto-cgit.txt
3235ed07ee20958bcc8aed49b5ab341260002cc0f37f57fe369264421cad62ea  /tmp/wf-epic-b/OXY-B004/round-12/presentproto-gitlab.txt
3235ed07ee20958bcc8aed49b5ab341260002cc0f37f57fe369264421cad62ea  /tmp/wf-epic-b/OXY-B004/round-12/presentproto-cgit.txt
$ cmp -s /tmp/wf-epic-b/OXY-B004/round-12/presentproto-gitlab.txt /tmp/wf-epic-b/OXY-B004/round-12/presentproto-cgit.txt && printf 'contents-identical=yes\n'
contents-identical=yes
```

The rendered AT-SPI page identifies `atspi/atspi-text.c` as its source through its `src` link. The sidecar therefore records the source repository, the pinned 2.60.0 commit, that repository-relative path, and the direct raw retrieval rather than treating the GitLab Pages host as a source repository. The direct raw response was 34,587 bytes with SHA-256 `69aa0554bcf4994864296670bac682a8d5b090917681075ec85b3b0b02801fdf`.

```text
$ curl -sS -fL --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-12/atspi-text-interface-rendered.html -w 'http=%{http_code}\n' https://gnome.pages.gitlab.gnome.org/at-spi2-core/libatspi/iface.Text.html
http=200
bytes=20233 sha256=c47154454768a9484fffc17b5e4a2db44758aee7981eb3c5df26bf594a421d53
$ grep -n -E 'Generated|atspi-text\\.c' /tmp/wf-epic-b/OXY-B004/round-12/atspi-text-interface-rendered.html
204:        <p>Generated by <a href="https://gitlab.gnome.org/GNOME/gi-docgen">gi-docgen</a> 2026.2</p>
227:          <a class="srclink" title="go to source location" href="https://gitlab.gnome.org/GNOME/at-spi2-core/-/blob/HEAD/atspi/atspi-text.c#L27">[src]</a>
$ curl -sS -fL --max-time 60 -o /tmp/wf-epic-b/OXY-B004/round-12/atspi-text.c -w 'http=%{http_code}\n' https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/atspi/atspi-text.c
http=200
bytes=34587 sha256=69aa0554bcf4994864296670bac682a8d5b090917681075ec85b3b0b02801fdf
$ grep -n -m 4 -E 'AtspiText:|atspi_text_get_text:|number of bytes' /tmp/wf-epic-b/OXY-B004/round-12/atspi-text.c
28: * AtspiText:
103: * atspi_text_get_text:
108: * Gets a range of text from an #AtspiText object.  The number of bytes
```

The Ubuntu intellectual-property policy was fetched through the reader proxy. Its content is normalized reader text, so this report intentionally records no full-body digest.

```text
$ curl -sS -fL --max-time 60 https://r.jina.ai/https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy -o /tmp/wf-epic-b/OXY-B004/round-10/ubuntu-ip-policy.txt
exit=0 bytes=52654
1:Title: Intellectual property rights policy | Terms and policies | Ubuntu and Canonical Legal | Canonical
```
...
- Retention in `.constitution/tech-spec/contracts/qualification-lock.json`: this spike adds no string to `preImplementationKnownUnknowns` or `gatingKnownUnknowns` (delta +0); every retained table 1 KU is covered by an existing binding in `crates/oxyflut-qualification/src/readiness.rs`.
  - `Native X server and extension floor` -> `minimum-platform-and-protocol-versions`.
  - `Complete IBus GTK input-method transcript for each allocation` -> `complete-ime-editing-geometry-and-accessibility-maps`.
  - `Linux assistive technology and base AT-SPI surface` -> `complete-ime-editing-geometry-and-accessibility-maps`.
  - `Focused allocation forward and reverse AT-SPI map` -> `complete-ime-editing-geometry-and-accessibility-maps`.
  - `Integrated allocation forward and reverse AT-SPI map` -> `complete-ime-editing-geometry-and-accessibility-maps`.
  - `Independent timing meter and per-output association` -> `independent-presentation-opportunity-sources`.
  - `Focused allocation interface inventory and service routing` -> `capability-and-platform-baselines`.
  - `Integrated allocation interface inventory and service routing` -> `capability-and-platform-baselines`.
...
The sidecar convention follows `qualification/schemas/external/README.md` and the existing `qualification/schemas/external/dsse-envelope-v1/spec/protocol.source.json` file. Every sidecar sets `kind` to `authoritative`, records the exact retrieved identity, and sets `sha256` to the streamed SHA-256 of its adjacent fixture, which must equal every evidence-object digest for that fixture. Every `path` and `licenseSource.path` is upstream-repository-relative; no sidecar may use a local capture filename as either value. For the eight SPDX-licensed fixtures, write fields in this order: `kind`, `repository`, `commit`, `path`, `retrievalUrl`, `license`, `licenseSource`, `version`, and `sha256`. Their `licenseSource.path` is `COPYING` at the recorded upstream commit. For the six Ubuntu package or source-package page fixtures, insert `licenseNote` and `licenseUrl` immediately after `license`. The five `packages.ubuntu.com` package-page sidecars use `repository` `https://packages.ubuntu.com`, a `path` of `resolute/<package>`, `license` `LicenseRef-page-copyright-notice`, `licenseUrl` `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`, `licenseSource` `{"repository":"https://ubuntu.com","path":"legal/terms-and-policies/intellectual-property-policy"}`, and the identical required `licenseNote` in table 3. The Launchpad source-package page and Ubuntu release-notes page use the same upstream-relative Ubuntu intellectual-property-policy `licenseSource` rather than a local capture filename. Use JSON `null` for an unavailable publisher source revision; don't invent a commit.

The fetched [GTK COPYING file](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/COPYING) and [AT-SPI COPYING file](https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/COPYING) contain the GNU Lesser General Public License version 2.1 terms with the later-version option, so their fixtures use SPDX `LGPL-2.1-or-later`. The fetched [X.Org COPYING file](https://cgit.freedesktop.org/xorg/proto/presentproto/plain/COPYING?id=bfdc7e052302c79c5803ad95a73c9b63b350c40c) contains the X.Org permission notice, so its fixture uses SPDX `X11`, never the non-SPDX hybrid `MIT/X11`. The fetched [Ubuntu intellectual-property policy](https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy) distinguishes Ubuntu package and binary-file copyright from the licenses of individual components. Therefore each package-page capture uses `LicenseRef-page-copyright-notice`, cites the policy as the license source, and does not claim the package payload's license.
...
| :-- | :-- | :-- |
| `s01-ubuntu-libgtk-4-1.html`; `s01-ubuntu-libgtk-4-1.html.source.json` | `https://packages.ubuntu.com`; `null`; `resolute/libgtk-4-1`; `https://packages.ubuntu.com/resolute/libgtk-4-1`; `4.22.2+ds-1ubuntu1` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `Ubuntu package page captured as version-identity evidence only; content is Canonical Ltd. copyright under the Ubuntu intellectual property policy and is not redistributed as a software artifact.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |
| `s02-ubuntu-gtk4-source-package.html`; `s02-ubuntu-gtk4-source-package.html.source.json` | `https://launchpad.net`; `null`; `ubuntu/+source/gtk4/4.22.2+ds-1ubuntu1`; `https://launchpad.net/ubuntu/+source/gtk4/4.22.2+ds-1ubuntu1`; `4.22.2+ds-1ubuntu1` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `Ubuntu package page captured as version-identity evidence only; content is Canonical Ltd. copyright under the Ubuntu intellectual property policy and is not redistributed as a software artifact.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |
...
| `s07-presentproto.txt`; `s07-presentproto.txt.source.json` | `https://gitlab.freedesktop.org/xorg/proto/presentproto`; `bfdc7e052302c79c5803ad95a73c9b63b350c40c`; `presentproto.txt`; `https://gitlab.freedesktop.org/xorg/proto/presentproto/-/raw/bfdc7e052302c79c5803ad95a73c9b63b350c40c/presentproto.txt`; `1.1` | `license`: `X11`; `licenseSource.path`: `COPYING`; `licenseSource.commit`: `bfdc7e052302c79c5803ad95a73c9b63b350c40c`. |
| `s08-ubuntu-at-spi2-core.html`; `s08-ubuntu-at-spi2-core.html.source.json` | `https://packages.ubuntu.com`; `null`; `resolute/at-spi2-core`; `https://packages.ubuntu.com/resolute/at-spi2-core`; `2.60.0-1` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `Ubuntu package page captured as version-identity evidence only; content is Canonical Ltd. copyright under the Ubuntu intellectual property policy and is not redistributed as a software artifact.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |
| `s09-atspi-text-interface.html`; `s09-atspi-text-interface.html.source.json` | `https://gitlab.gnome.org/GNOME/at-spi2-core`; `d8ab833f0230fccc009c271d9f23f53df2a32c88`; `atspi/atspi-text.c`; `https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/atspi/atspi-text.c`; `2.60.0` | `license`: `LGPL-2.1-or-later`; `licenseSource.path`: `COPYING`; `licenseSource.commit`: `d8ab833f0230fccc009c271d9f23f53df2a32c88`. |
...
| `s12-ubuntu-ibus-gtk4.html`; `s12-ubuntu-ibus-gtk4.html.source.json` | `https://packages.ubuntu.com`; `null`; `resolute/ibus-gtk4`; `https://packages.ubuntu.com/resolute/ibus-gtk4`; `1.5.34~rc2-1` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `Ubuntu package page captured as version-identity evidence only; content is Canonical Ltd. copyright under the Ubuntu intellectual property policy and is not redistributed as a software artifact.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |
| `s13-ubuntu-xserver-xorg-core.html`; `s13-ubuntu-xserver-xorg-core.html.source.json` | `https://packages.ubuntu.com`; `null`; `resolute/xserver-xorg-core`; `https://packages.ubuntu.com/resolute/xserver-xorg-core`; `2:21.1.22-1ubuntu1` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `Ubuntu package page captured as version-identity evidence only; content is Canonical Ltd. copyright under the Ubuntu intellectual property policy and is not redistributed as a software artifact.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |
| `s14-ubuntu-orca.html`; `s14-ubuntu-orca.html.source.json` | `https://packages.ubuntu.com`; `null`; `resolute/orca`; `https://packages.ubuntu.com/resolute/orca`; `50.1.2-1ubuntu1` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `Ubuntu package page captured as version-identity evidence only; content is Canonical Ltd. copyright under the Ubuntu intellectual property policy and is not redistributed as a software artifact.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |
| `s15-ubuntu-26.04-release-notes.html`; `s15-ubuntu-26.04-release-notes.html.source.json` | `https://documentation.ubuntu.com`; `null`; `release-notes/26.04/`; `https://documentation.ubuntu.com/release-notes/26.04/`; `26.04 LTS (Resolute Raccoon)` | `license`: `LicenseRef-page-copyright-notice`; `licenseNote`: `The captured publisher page contains a copyright notice and does not state a page-content license.`; `licenseUrl`: `https://ubuntu.com/legal/terms-and-policies/intellectual-property-policy`; `licenseSource.repository`: `https://ubuntu.com`; `licenseSource.path`: `legal/terms-and-policies/intellectual-property-policy`. |

The GTK, AT-SPI, and X.Org release commits in table 3 come from the preserved discovery probe. Table 3 records the upstream `COPYING` path for every SPDX fixture and the upstream Ubuntu intellectual-property-policy path for every page-copyright notice. It does not govern the Wayland map.
...
- [AT-SPI Text documentation source 2.60.0](https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/atspi/atspi-text.c)
- [AT-SPI Text D-Bus XML 2.60.0](https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/xml/Text.xml)
- [GTK 4.22.2 AT-SPI text bridge source](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/a11y/gtkatspitext.c)
- [GTK 4.22.2 accessible text implementation](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/gtkaccessibletext.c)
- [GTK 4.22.2 GtkAccessibleText API documentation](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/gtk/gtkaccessibletext.h)
...
- [Present protocol specification](https://gitlab.freedesktop.org/xorg/proto/presentproto/-/raw/bfdc7e052302c79c5803ad95a73c9b63b350c40c/presentproto.txt)
...
- [GTK COPYING file](https://gitlab.gnome.org/GNOME/gtk/-/raw/5957885ec3c8c089965fc73ed2b882e605a1817e/COPYING)
...
- [AT-SPI COPYING file](https://gitlab.gnome.org/GNOME/at-spi2-core/-/raw/d8ab833f0230fccc009c271d9f23f53df2a32c88/COPYING)
