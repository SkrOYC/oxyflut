...
| T3.4c | `.constitution/tech-spec/stack.md` | `Scope guard` paragraph beginning `The current qualification lock`: append SPK-B005's exact unscored-probe allowance that limits the probe to resolving a pre-implementation KU and excludes comparative scores and selection. | Exact Scope guard sentence review and `contracts validate` traceability family. | [SPK-B005, Spec edits required](../spikes/SPK-B005.md#spec-edits-required) | Stage 3 |
