...
- Ticket: OXY-D001
- Status: reconciliation complete through the [Stage 3 reconciliation checklist](#stage-3-reconciliation-checklist); readiness not set
...
Table 1 classifies every committed pre-implementation KU and both gating-only source-pin KUs. Each cited result is one committed path and each owner is singular. The code's `upstream_owner` values name archived Epic A/C tickets; this report reassigns each to the stage or ticket that can still act.
...
| T3.5 | `.constitution/tech-spec/changelog.md` and CI advisory configuration | Add the accessibility-map and qualification-lock migration notes; bind a pinned offline advisory database and refresh policy; assign remaining baseline owners; retain library-only template APIs pending their contract. | `baseline validate`, `measurement validate`, `generate_templates`, and `digests::validate_workspace`. | [Epic D inputs from Epics A and C](../tasks/active/EPIC-D-readiness-reconciliation.md#oxy-d001-inputs-from-epics-a-and-c) and [SPK-B001, Spec edits required](../spikes/SPK-B001.md#spec-edits-required) | Stage 3 |
...
| T5.2a | `crates/oxyflut-qualification/src/readiness.rs` | Reassign the `POLICY_FIELDS` and `KNOWN_UNKNOWN_BINDINGS` `upstream_owner` values that differ from Tables 1 and 3 to the listed Stage 3, OXY-B007, or OXY-B008 owner. | `readiness.rs` `POLICY_FIELDS` and `KNOWN_UNKNOWN_BINDINGS` tables. | `crates/oxyflut-qualification/src/readiness.rs:45-215` | Stage 3 |
...
1. Stage 3 completes the [Stage 3 reconciliation checklist](#stage-3-reconciliation-checklist) and verifies every readiness-blocking input before removing it from the exact KU set.
2. Every readiness-blocking input is supplied, verified, and removed from the exact KU set before the flag is set; `xtask/src/contracts/readiness.rs:225-231` rejects a `candidateImplementationReady` claim while `candidate_input_issues` is nonempty. A Stage 1 deferral leaves readiness false unless a separately approved upstream scope change revises the governing requirements and downstream contracts.
