...
Table 1 classifies every committed pre-implementation KU and both gating-only source-pin KUs. Each cited result is one committed path and each owner is singular. The code's `upstream_owner` values name archived Epic A/C tickets; this report assigns artifact creation and binding to Stage 3, workload and scoring-anchor/assessor corpus definition to the next Stage 4 epic, and hardware or assessor inputs to OXY-B007 or OXY-B008.
...
| :-- | :-- | :-- | :-- | :-- |
| External-fixture `source.json` sidecar validator ownership | `xtask/src/commands/external_contracts.rs:45-94` runs only `SNAPSHOTS`, and `xtask/src/contracts/digests.rs:214-257` scans only contract JSON. | Adopt a new `external-contracts verify` sidecar-validation pass. | Add a sidecar-validation pass to `external-contracts verify` for every regular same-stem `qualification/fixtures/external-contracts/**/*.source.json` sidecar: require a regular sibling, equal sibling SHA-256, canonical retrieval URL, upstream-relative `path` and `licenseSource.path`, and required license fields. | Stage 3 |
...
| # | File | Field, section, or symbol | Enforcing check | Source anchor | Owner |
| :-- | :-- | :-- | :-- | :-- | :-- |
| T2.6.1 | `xtask/src/commands/external_contracts.rs` | `run` and a new external-fixture-sidecar validation pass: validate regular sibling bytes, equal SHA-256, canonical retrieval URL, upstream-relative `path` and `licenseSource.path`, and required license fields. | `cargo run -q -p xtask -- external-contracts verify` with the new sidecar-validation pass. | [Epic D external-fixture input](../tasks/completed/EPIC-D-readiness-reconciliation.md#oxy-d001-inputs-from-epic-b) | Stage 3 |
...
| T5.2a | `crates/oxyflut-qualification/src/readiness.rs` | Reassign the `POLICY_FIELDS` and `KNOWN_UNKNOWN_BINDINGS` `upstream_owner` values: Stage 3 for artifact creation and binding, Stage 4 (next epic via /planning-engineering-execution) for workload and scoring-anchor/assessor corpus definition, and OXY-B007 or OXY-B008 for external inputs. | `readiness.rs` `POLICY_FIELDS` and `KNOWN_UNKNOWN_BINDINGS` tables. | `crates/oxyflut-qualification/src/readiness.rs:45-215` | Stage 3 |
