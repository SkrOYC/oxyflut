...
- Maintain a pinned Flutter Engine fork. The fork replaces Dart's runtime controller with a Rust runtime controller and retains the Flutter shell, rasterizer, compositor, and platform embedders. Starling is an example of this architecture with Swift as the framework language.
...
Path B introduces a runtime controller interface between the Flutter shell and the framework implementation. A C callback table delivers frame, view, input, semantics, locale, lifecycle, and platform-message events to Rust. Starling is an example that demonstrates this runtime substitution with Swift.
...
| K-03 | KK | Runtime substitution can retain the Flutter shell; Starling is an example that demonstrates this architecture with a Swift-specific callback contract. | Use the example as evidence for feasibility, not as the `oxyflut` interface. |
