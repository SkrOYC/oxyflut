...
- Qualification proceeds in a declared sequence so evidence from the environment and substrate candidate the project can build arrives first.
- Readiness gates protect measurement comparability, not shared implementation work that doesn't depend on a rendering substrate.
...
The operator prefers a safe Rust public API and production binaries that don't start a Dart virtual machine or execute Dart code. The rendering foundation must be selected between two version-pinned Flutter-derived families: the standalone Impeller C SDK or a full Flutter Engine with a language-neutral runtime boundary. The latter replaces the application runtime while retaining selected engine subsystems. Starling is an example of this. It demonstrates runtime substitution with Swift. Tier 1 targets are macOS, Windows through Win32, and Linux through both Wayland and X11. These preferences guide later technical stages but don't replace the product requirements. The operator's declared bias is that the integrated candidate is qualified first and Linux is the first qualification environment.

The available Linux reference host, `thinkpadp14s`, is a single x86_64 machine with an AMD integrated GPU running NixOS in a Hyprland Wayland session, with X11 through Xwayland for interactive qualification and Xvfb for headless qualification. As a non-binding platform hint, the operator prefers whole-system Nix pinning of that host to a distribution package snapshot and accepts the recorded risk that this session family is not a mainstream desktop session. Tier 1 environments are qualified in this order: Wayland, X11, macOS, then Windows.
