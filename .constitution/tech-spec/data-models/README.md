...
| `diagnostic-event-registry.schema.json` | Freezes stable event names, classifications, and allowed fields. | Event additions require registry review; field meaning changes require a major version. |
...
Schema validation is necessary but not sufficient. The `xtask contracts validate` command must resolve only the local schema registry and verify file existence and SHA-256 bindings; the exact PRD and architecture ID sets; traceability symbols and contract tests; diagnostic names and fields against the registry; every capability baseline; every external schema snapshot; score arithmetic and assessor consensus; zero-, one-, and two-candidate selection arithmetic; and every Phase 3B promotion reference. Promotion validation must bind the selected candidate, qualification evidence, decision, layout corpus and result, accepted ADR-0010 bytes, final contract set, target matrix, all-Tier-1 results, losing-candidate removal, production bill of materials, and release qualification to the same qualification lock and Stage 3 version. A schema-valid document with a missing, mismatched, or fabricated reference fails validation.
