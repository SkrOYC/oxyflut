...
| :-- | :-- | :-- |
| `accessibility-map.schema.json` | Records one candidate's complete forward property map and reverse action map for one environment. | `kk-complete` requires every forward mapping and reverse action to be KK; indexed reverse actions bind to an immutable text-layout generation; any native mapping change reruns API inspection and end-to-end assistive-technology tests. |
...
| `capability-baseline.schema.json` | Freezes the exact test vectors and evidence expected for all 52 P0 capabilities. | A baseline change creates a new lock and prevents comparison with the old corpus; cross-file validation requires its `specificationVersion` to equal the active specification. |
| `capability-traceability.schema.json` | Maps every P0 capability to its architecture flow, target crate, and physical contracts. | Every Stage 3 revision must keep exactly one mapping per P0 capability; cross-file validation requires its `specificationVersion` to equal the active specification. |
...
Platform IME index units map exactly to the native ABI constants: `utf8-bytes` is 1, `utf16-code-units` is 2, and `unicode-scalars` is 3. Both substrate adapters must reject every other numeric value before converting a range.
