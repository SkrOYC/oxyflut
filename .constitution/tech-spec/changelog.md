...
- T1.4a: `.constitution/tech-spec/data-models/qualification-lock.schema.json`, `crates/oxyflut-qualification/src/readiness.rs`, `xtask/src/contracts/readiness.rs`, and `xtask/src/commands/lock_tests.rs`; make readiness per environment and move `measurementPolicy.scoringAnchors` and `measurementPolicy.assessors` from candidate implementation to conditional two-entered-candidate measurement readiness; enforce with `LOCK_SCHEMA`, `candidate_input_issues`, `POLICY_FIELDS`, `KNOWN_UNKNOWN_BINDINGS`, and the `lock_tests.rs` exact sets; see ADR-0011.
...
- T3.3d.1: `xtask/src/commands/{lock,lock_tests}.rs`; add `--environment ENVIRONMENT` to the candidate-implementation and measurement `lock status` gates, including all-environment reporting without that flag and the 0 ready, 2 valid-but-open, and 1 invalid exit contract; enforce with `lock_tests.rs` per-environment status assertions; see ADR-0011.
