...
fn validate_interface(
    ...
    let ast = ast::parse_header_ast(header, tools)?;
...
fn validate_enum_constants(
    ...
    ast::collect_enum_constants(ast, prefix, &mut actual);
    if actual != expected {
        return Err(NativeContractError::InterfaceMismatch {
...
fn validate_layout(
    ...
    ast::derive_header_nullability(header, tools, &mut actual)?;
...
/// Loads the manifest-verified Rust compiler without requiring native host support.
///
/// # Errors
///
/// Returns an error when the staged manifest or its `rustc` entry is missing, substituted, or
/// inconsistent with the pinned Rust toolchain.
pub(crate) fn load_rust_contract_compiler(root: &Path) -> Result<PathBuf, NativeContractError> {
    load_manifest(root)?
        .verified_executable_path("rustc")
        .map_err(NativeContractError::Toolchain)
}

fn load_manifest(root: &Path) -> Result<ToolchainManifest, NativeContractError> {
